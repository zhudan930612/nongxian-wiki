...
不再手工搬迁，而是调用确定性脚本 `finalize_ingest.py` 一次完成：原始 PDF + mineru 产物（`00-收件箱/PDF解析/<stem>/`）搬到归档位、图片目录重命名与引用改写、frontmatter 定稿、插入原始资料索引、清理空壳。`<stem>` 为原 PDF 文件名去扩展名。
...
脚本行为（对应旧手工步骤，全部相对 vault 根）：
0. 原件 `00-收件箱/<stem>.pdf` → `10-原始资料/99-PDF原件/{分类}/YYYYMMDD-原标题.pdf`
1. 全文 md → `10-原始资料/{分类}/YYYYMMDD-原标题.md`（供 Grep 检索）
2. 图片目录 `<token>.images/` → 重命名 `YYYYMMDD-原标题.images/` → `99-PDF原件/{分类}/`
3. 图片引用 `![图 N](<token>.images/x)` → `![[99-PDF原件/{分类}/YYYYMMDD-原标题.images/x]]`
4. frontmatter：`title: 全文·原标题`、`source_pdf` 指向归档 PDF、`extraction_quality` 定为 `--quality` 值、保留 `extracted_by: mineru`
5. 正文开头插入 `> 📎 **原始资料**：[[99-PDF原件/{分类}/YYYYMMDD-原标题.pdf]]`
6. 删除 `00-收件箱/PDF解析/<stem>/` 空壳
7. stdout 打印 JSON（归档路径、图片引用数、孤立图片删除数、quality），据此核对

**孤立图片规则**（脚本 `--images` 参数）：
- `auto`（默认）：md 图片引用数 > 0 → 搬迁并改写；引用数 = 0 且图片目录非空（多为公式/表格的冗余渲染）→ **删除图片目录**，JSON 报告 `orphan_dropped`（PDF 原件已保全部内容，可兜底）
- `keep`：疑似含真实图表却未被引用时，强制搬迁保留图片目录，人工补引用
- `drop`：强制删除图片目录

**防覆盖**：目标 PDF / md / images 已存在时脚本报错退出，不覆盖；需先清理或更换 `--name`。
...
4. 定好分类/命名/标题后，运行 `finalize_ingest.py` 一键搬迁三件套（原件 → `99-PDF原件/{分类}/`；全文 md → `10-原始资料/{分类}/`，开头插入原始资料索引；图片重命名移入 `99-PDF原件/{分类}/` 并改写为 `![[...]]`；孤立图片默认删除；frontmatter 定稿）（分类取自 7 类：政策法规/学术论文/行业报告/技术规范/产品与公司/媒体报道/会议沟通）
